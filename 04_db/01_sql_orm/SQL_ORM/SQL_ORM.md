...
2. user 레코드 생성
   ...
   ```sql
   -- sql
   INSERT INTO users_user VALUES (101)
   ```
...
6. 나이가 30이거나 성이 김씨인 사람?   
   ...
   > or 은 `Q 와 |` 를 사용하여 표현한다.

   > print(User.objects.filter(Q(age=30) | Q(last_name='김')).query = query문 확인
   ...
   ```sql
   -- sql
   SELECT * FROM user_user WHERE age=30 or last_name = '김';
   ```
...
8. 거주 지역이 강원도이면서 성이 황씨인 사람의 이름
   ...
   ```python
   # orm
   User.objects.filter(country='강원도', last_name='황').values('first_name')  (index 접근)
   
   .first().get('first_name') (이름만 출력)
   ```
...
1. 나이가 많은 사람순으로 10명
   ...
   > 기본적으로 오름차순이기에 -age 로 역순으로 만든 후 10명만 추출
...
2. 잔액이 적은 사람순으로 10명
   ...
   ```python
   # orm
   User.object.order_by('balance')[:10]
   ```
